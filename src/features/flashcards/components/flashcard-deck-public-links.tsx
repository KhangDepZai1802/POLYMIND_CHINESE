...
/**
 * Bảng "địa chỉ QR của cả bộ thẻ" (`D-39`).
 *
 ...
 *
 * 🔴 Từ `MULTIDECK-1` khối này nằm trong **ngăn kéo**, không còn ở giữa dòng
 * chảy của trang. Lý do là số đo thô: 35 buổi × 1 dòng đẩy khu soạn thẻ xuống
 * dưới màn hình đầu tiên, tức việc làm HẰNG NGÀY bị việc làm MỖI KỲ IN che mất.
 * Nội dung bên trong giữ nguyên từng chữ — chỉ chỗ đặt là đổi.
 */
export function FlashcardDeckPublicLinks({
  ...
  /**
   * Mã BỘ, không phải mã khoá (`MULTIDECK-1`). Truyền nhầm mã khoá thì bảng này
   * hiện ra một dải địa chỉ mà DB sẽ không bao giờ phát hành.
   */
  deckCode: string;
}) {
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-text-secondary text-sm">
          Mã cố định theo <strong>mã bộ</strong> và số buổi, biết trước được nên
          đưa cho bên in ngay cả khi buổi còn nháp. Buổi chưa công bố thì người
          quét thấy trang “sắp mở”, công bố xong là nội dung tự hiện — không phải
          đổi mã.
        </p>
        <StatusBadge
          label={
      ...
      {unresolvable ? (
        <p className="text-danger-ink text-sm">
          Mã bộ “{deckCode}” không có chữ hoặc số nào nên không tạo được mã liên
          kết. Hãy sửa mã bộ trước.
        </p>
      ) : null}
      ...
      {/*
        ...
        chỉ dài đẩy cả ngăn kéo tràn ngang trên điện thoại.
      */}
