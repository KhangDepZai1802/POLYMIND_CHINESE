...
import { reportFilterSearchParams, type AdminReportFilters } from "../schema";
...
/**
 * Tab HỌC PHÍ của `/admin/reports` (D1) — nguyên trạng màn báo cáo học phí cũ,
 * chỉ chuyển từ page vào component để nhường mặc định cho tab Học tập.
 * Nghiệp vụ, filter và export (`report=tuition`) KHÔNG đổi — link export cũ
 * đã lưu ở đâu đó vẫn chạy.
 */
export function TuitionReport({
  filters,
  errorMessage,
  report,
  classes,
      ...
      <Card className="mb-4 gap-4 py-4">
        ...
        <CardContent className="px-4">
          <form className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            {/* Áp dụng filter xong phải Ở LẠI tab Học phí. */}
            <input type="hidden" name="tab" value="hoc-phi" />
            ...
            <div className="flex items-end gap-2">
              ...
              <Button asChild variant="ghost">
                <Link href="/admin/reports?tab=hoc-phi">Xóa lọc</Link>
              </Button>
            </div>
          </form>
          {errorMessage && (
