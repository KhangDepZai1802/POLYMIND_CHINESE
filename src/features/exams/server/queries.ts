...
export async function getExamGradingData(deliveryId: string) {
  ...
  const { data, error } = await supabase
      ...
      `id,title,status,results_published_at,class:classes(code,name),
       attempts:exam_attempts(
         ...
         answers:exam_answers(
           ...
           item:question_set_items(points,order_index,question_version:question_versions(id,question_type,prompt_text,options:question_options(option_key,content,order_index)))
         ),
    ...
    .maybeSingle();
  if (error)
    throw new Error(`Không tải được dữ liệu chấm thi: ${error.message}`);
