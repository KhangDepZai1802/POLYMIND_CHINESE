...
`session_number` không vượt `courses.default_session_count`; Course chưa chốt số buổi thì chưa publish section. Publish một buổi yêu cầu đúng một trang `session_cover` đủ hai ảnh, ít nhất một trang `vocabulary`, mọi vocabulary có audio và thứ tự liên tục.
