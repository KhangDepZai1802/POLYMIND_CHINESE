...
`session_number` không vượt `courses.default_session_count`; Course chưa chốt số buổi thì chưa publish section. Publish một buổi yêu cầu đúng một trang `session_cover`, ít nhất một trang `vocabulary`, thứ tự liên tục và mọi trang đủ media.

#### `flashcard_pages`

`id` uuid PK · `section_id` uuid NOT NULL FK → `flashcard_sections` ON DELETE CASCADE · `kind` `flashcard_page_kind` NOT NULL · `order_index` int NOT NULL · `term` text · `front_image_path` / `back_image_path` / `audio_path` text NOT NULL · `front_alt` / `back_alt` text NOT NULL · `created_by` uuid NOT NULL FK → `auth.users` · `archived_at` timestamptz · timestamps · partial UNIQUE `(section_id, order_index)` cho page active; partial UNIQUE `(section_id)` WHERE `kind = 'session_cover' AND archived_at IS NULL`.

Trang mở đầu có `order_index = 0`; trang từ vựng có `order_index > 0`. Path do server sinh trong bucket `flashcard-media`, không nhận namespace tùy ý từ client.
...
Migration thực thi: `20260721000066_flashcards.sql` và `20260721000067_wrong_answer_review.sql`. Helper media câu sai chỉ mở metadata/object `question-media` khi queue của chính học viên còn unresolved; resolve xong thì signed URL mới bị từ chối.
...
| Flashcard deck/section/page | ALL + publish RPC     | **DENY**                                                                               | SELECT: Course lớp mình học + section/deck đã publish                    |
...
| `publish_flashcard_section(section_id)`                                       | Khóa section, kiểm số buổi + cấu trúc cover/pages + đủ 2 ảnh/audio, đặt publish và audit trong một transaction                                                                                                                                    |
...
| `flashcard-media`   | ✅      | admin + student có Course tương ứng và section đã publish                           | super admin qua signed ticket          |
...
- Flashcard dùng path `{admin_uid}/{deck_id}/{section_id}/{page_id}/{slot}-{uuid}.{ext}`; `slot` chỉ `front`, `back`, `audio`. Server ký upload sau khi kiểm role/namespace/MIME/size, rồi đọc metadata Storage thật trước khi lưu page. Student chỉ được ký object đang được một page active của section/deck đã publish và Course thuộc enrollment của mình.
