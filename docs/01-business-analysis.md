...
| 5 | Lớp học | Mở lớp, sĩ số, hình thức, địa điểm, phân công giáo viên chính/trợ giảng |
...
| 9 | Bài tập | Giao bài, đính kèm file, hạn nộp; học viên nộp text/file; giáo viên chấm + phản hồi |
| 10 | Kiểm tra & đánh giá | Điểm 0–100 + điểm 6 kỹ năng; xếp loại theo thang cấu hình; nhận xét định kỳ |
...
| Bài tập | CRUD | CRUD (assigned) | R (assigned + đã publish) |
| Bài nộp | R + U (chấm) | R + U (chấm, assigned) | C + U (own, chỉ nội dung/file; **không** sửa điểm/feedback) |
| Bài kiểm tra & kết quả | CRUD | CRUD (assigned) | R (own + **đã publish**) |
...
- Mỗi lớp **tối đa 1 giáo viên chính** (`primary`), không giới hạn số trợ giảng (`assistant`). Ràng buộc bằng partial unique index ở DB.
...
- Bài tập chỉ hiện với học viên khi đã `published_at`.
- Nộp sau `due_at` → đánh dấu `is_late`. Nếu `allow_late_submission = false` thì chặn nộp.
- Học viên chỉ nộp cho **enrollment của chính mình**, vào assignment **đã publish** thuộc **lớp mình đang học**. Ba điều kiện, kiểm cả ở server lẫn RLS.
- Điểm bài tập: `0 ≤ score ≤ max_score`. Học viên **không** sửa được `score`, `feedback`, `graded_by`.

### BR-7 — Kiểm tra, xếp loại và đánh giá

- Điểm tổng thang **0–100**. Điểm 6 kỹ năng (nghe, nói, đọc, viết, từ vựng, ngữ pháp) cũng 0–100, cho phép null.
...
| Code | Tên | `course_type` | Level |
|---|---|---|---|
| `HSK1`…`HSK6` | Tiếng Trung HSK 1…6 | `hsk` | HSK 1…6 |
| `GT` | Tiếng Trung giao tiếp | `communication` | null |
| `TN` | Tiếng Trung thiếu nhi | `kids` | null |
| `LT-HSK` | Luyện thi HSK | `exam_prep` | null (cấu hình target level theo lớp) |
...
- Đối tượng: Ban Giám đốc · Loại: `business_custom` · 35 buổi × 90 phút.
...
- Đối tượng: Lãnh đạo phòng / Cán bộ nhân viên · Loại: `business_custom` · 35 buổi × 90 phút · Sĩ số tối đa 26 · `offline` tại `108 Tây Thạnh, Phường Tây Thạnh, TP. HCM`.
