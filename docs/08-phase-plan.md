...
| TEACHER-REPORT-1i | **Đo bằng trình duyệt thật** | 360/375/390/414px trên 3 màn mới, **đếm phần tử con có `overflow-x` cuộn được** chứ không dùng `documentElement.scrollWidth` (bài học đắt nhất của `UX-MOBILE-1`) | ☑ **DONE** (phiên 104) — `tests/e2e/session-reports.spec.ts` **3/3**; kiểm ngược cho bảng 11 cột hiện ở màn hẹp ⇒ bắt được `div[overflow-x-auto] 1126>326` @360px trong khi `documentElement.scrollWidth` **vẫn là 0** |
| TEACHER-REPORT-1j | **Cổng pgTAP đầy đủ sau khi thêm bảng/RPC/bucket** | Chạy **CẢ BỘ** `supabase test db`, không chỉ file test của tính năng: `rls_catalog_matrix` là bài đếm catalog, mỗi object mới phải được review và bump mốc | ☑ **DONE** (phiên 104) — bộ đầy đủ từng **đỏ 5 bài** mà phiên 103 không thấy vì chỉ chạy hai file mới. Nay **748/748** |
| TEACHER-REPORT-2 | **Mục 3 "Bài học đã dạy" — giáo viên tự nhập** | Bỏ dropdown tra giáo trình; lưu vào `session_reports.lesson_title`. Gỡ **ba tầng** khoá cứng nút Gửi: (a) mục 3 tính theo chữ gõ tay chứ không theo `lesson_id`; (b) `save_session_log` bỏ cổng "phải chọn bài học mới hoàn tất" ở DB (luồng nhật ký riêng vẫn tự đòi ở tầng app); (c) chỉ ghi `lesson_progress` khi thật sự có bài học — cột `lesson_id` là NOT NULL | ☑ **DONE** (phiên 105) — `…093` **đã áp cloud**, xác minh bằng `psql` trong `BEGIN READ ONLY` |

| TEACHER-REPORT-3a | **Mục 8 — bản in/DOCX phải mang theo CHÍNH các tệp** | `ReportForRender` đổi `evidenceCount: number` → `evidence: RenderEvidence[]`, gắn vào `section.images` nên **cả ba bề mặt** cùng có ảnh. Bản xem/in nhúng URL ký hạn **10 phút**; DOCX nhúng nhị phân. `<img>` trần + `loading="eager"` chứ KHÔNG `next/image` — tải lười thì bấm Ctrl+P lúc ảnh chưa vào khung nhìn là in ra ô trắng | ☑ **DONE** (phiên 106) |
| TEACHER-REPORT-3b | **WebP → định dạng Word nhận được** | `docx` chỉ nhận `jpg\|png\|gif\|bmp`, mà bộ nén ở trình duyệt ra **WebP** — đây mới là gốc của "không hiển thị 3 tệp đính kèm". `server/evidence-images.ts` đọc kiểu THẬT của tệp rồi chuyển: **PNG nếu có kênh trong suốt** (ép JPEG là nền tách đổ đen), **JPEG cho ảnh chụp** (ép PNG là file Word phình gấp nhiều lần). Trần cạnh dài 1200px, **fail-open**: ảnh hỏng in một dòng nêu tên tệp, không đánh sập cả lượt xuất | ☑ **DONE** (phiên 106) — kiểm ngược (c) trả nguyên WebP vào `ImageRun` ⇒ e2e đỏ *"đang là webp"* |
| TEACHER-REPORT-3c | **Mục 2 — lý do vắng là TUỲ CHỌN** | Gỡ hẳn cổng "mỗi người vắng phải có một dòng lý do". `CompletionInput` **không còn khoá `attendance`** nên nối lại là typecheck đỏ; nhãn ghi rõ *"(không bắt buộc)"*. Cùng hình dạng sai với `TEACHER-REPORT-2`: **fail-closed chỉ đúng khi nó chặn thứ NGƯỜI DÙNG kiểm soát được** — giáo viên không biết vì sao học viên vắng | ☑ **DONE** (phiên 106) — kiểm ngược (b) trả về `partial` ⇒ đỏ đúng hai bài ghim |
| TEACHER-REPORT-3d | **Danh sách ảnh đọc thẳng từ máy chủ** | Lỗi thứ ba, bắt được lúc đo bằng trình duyệt: `useState(data.report.evidence)` + `router.refresh()` (**cố ý giữ state Client Component**) ⇒ tải ảnh xong màn hình vẫn *"0/4 ảnh"*, và mục 8 kẹt "chưa xong" nếu đã tick loại ảnh ⇒ nút Gửi bị chặn. Bỏ state, đọc thẳng prop | ☑ **DONE** (phiên 106) |
| TEACHER-REPORT-3e | **Đo bằng trình duyệt thật, trọn đường** | E2E đi đúng đường production: tải lên PNG **1400px** (vượt `FLASHCARD_IMAGE_MAX_EDGE` nên bộ nén buộc chạy) → ghim `storage_path` phải kết thúc `.webp` → bản in đo bằng `emulateMedia({media:"print"})` → tải DOCX rồi **mở zip đọc ngược header bằng `sharp`** | ☑ **DONE** (phiên 106) — Playwright `session-reports` **5/5** trên Chromium |

| TEACHER-REPORT-4a | **Bề ngang bảng DOCX tính bằng twip, không bằng phần trăm** | `<w:tblGrid>` là thứ Word DỰNG BẢNG THEO; không truyền `columnWidths` thì `docx` khai mỗi cột **100 twip ≈ 0,18 cm** ⇒ cả bảng co thành dải hẹp ở mép trái. Khai tuyệt đối: bảng `9639` twip (= 170mm lòng trang A4), cột `3663`/`5976`, kèm `layout: FIXED`. Vế `w:tblW w:type="pct" w:w="100%"` cũng sai kiểu — `ST_TblWidth` đòi số nguyên phần-năm-mươi-của-một-phần-trăm | ☑ **DONE** (phiên 107) — bài kiểm đọc THẲNG `word/document.xml`; kiểm ngược lùi về phần trăm ⇒ đỏ với đúng con số lỗi gốc *"tổng bề ngang cột = 200 twip"* |
| TEACHER-REPORT-4b | **Dòng "Thời gian" của mục 1** | In `10/08/2026 – 09:30` vì `session.startsAt` là chuỗi đã định dạng thành NGÀY mà dòng giờ lại dùng chính nó làm mốc bắt đầu. Thêm `session.startTime` riêng. Sai một chỗ thì sai **cả ba bề mặt** — đúng cái giá phải trả của "một nguồn cho ba bề mặt", và cũng là lý do chỉ cần sửa một chỗ | ☑ **DONE** (phiên 107) |
| TEACHER-REPORT-4c | **Cỡ chữ bản in** | 🔴 **Đo trước khi sửa:** ở đúng bề ngang A4 (794px CSS) bản in **không tràn** và chữ là **14px = 10,5pt** — cỡ tài liệu bình thường, KHÔNG phải bug; nhỏ vì đọc nguyên trang A4 trên màn điện thoại. Phần sửa được là khoảng trắng: giá trị **11,5pt**, nhãn **10pt**, thu lề dọc thẻ, cột nhãn 16rem → 13rem, neo bằng `[data-report-print]` để hai bản in kia không đổi | ☑ **DONE** (phiên 107) — đo lại: chữ **+9,5%** (14 → 15,33px) mà tổng chiều cao trang **−9,5%** (2549 → 2306px) |

**Cổng đã chạy thật (phiên 107):** lint **0** · typecheck **0** · Vitest **626/626** (621 → +5) · build **exit 0** · Playwright `session-reports` **5/5** trên Chromium. `TEACHER-REPORT-4` **KHÔNG có migration**. ⚠️ Đã đo bằng cách mở zip đọc `word/document.xml`, **chưa mở bằng Word thật** — vẫn cần một lượt xác nhận bằng mắt.

### Bốn việc user báo kèm ảnh — `TEACHER-REPORT-5` (user chốt 2026-08-17 qua `AskUserQuestion`)

| ID | Việc | Definition of Done | Trạng thái |
|---|---|---|---|
| TEACHER-REPORT-5a | **Số đề mục 1–9 chìm trên bản in** | Số là chữ trắng trên nền xanh, mà **trình duyệt bỏ màu nền khi in** ⇒ chữ trắng trên giấy trắng. ⛔ KHÔNG chữa bằng `print-color-adjust: exact` — cờ đó trình duyệt được phép bỏ qua, và bỏ qua thì con số **mất hẳn**, hỏng nặng hơn. Đảo thành **chữ mực đậm + viền**: viền và màu chữ luôn được in. Neo bằng `[data-report-number]` trong khối `@media print`, và bài kiểm phải ghim **cả vế ngược** (trên màn hình vẫn phải là chữ sáng trên nền đậm) | ☑ **DONE** (phiên 109) — kiểm ngược gỡ khối CSS ⇒ đỏ đúng con số *"nền phải trong suốt: expected 0, received 1"* |
| TEACHER-REPORT-5b | **Nút "Không cần báo cáo" cho giáo vụ** | Cờ đặt trên **`class_sessions`** (migration `…094`), không thêm `status='waived'` vào `session_reports` — buổi cần miễn thường **chưa có hàng báo cáo nào**. RPC `set_session_report_waiver` fail-closed theo `app.is_manager()` (**giáo viên của chính lớp đó gọi thẳng RPC cũng bị chặn**), chặn miễn buổi đã có báo cáo đã gửi, `report_waived_by = auth.uid()`, **idempotent** (bấm hai lần không ghi lại mốc, không thêm audit). ⚠️ Cổng ở DB **không** kiểm "đã điểm danh chưa" — buổi không cần báo cáo thường chính là buổi không ai điểm danh, và giáo vụ không kiểm soát việc điểm danh (bài học `TEACHER-REPORT-2`/`3c` lần thứ ba); điều kiện `attendanceDone` chỉ nằm ở **nút trên màn hình**, đúng yêu cầu user. Có **đường lùi** tại chỗ | ☑ **DONE** (phiên 109) — pgTAP `session_report_waiver` **15/15** |
| TEACHER-REPORT-5c | **Cờ miễn phải tới ĐƯỢC hàng đợi giáo viên** | Nút chỉ ở trang giáo vụ, nhưng buổi đã miễn phải rời khỏi nhóm việc-cần-làm của giáo viên, khỏi `getMissingReportCount()` và khỏi `isOverdue()` — không thì cái nút không làm được đúng việc nó nói. Nhưng **không xoá khỏi màn hình**: xuống nhóm riêng *"Không cần báo cáo"* kèm lý do, vì giáo viên không phải người bấm nút | ☑ **DONE** (phiên 109) — 🔴 kiểm ngược lần đầu **KHÔNG đỏ**; phải thêm phép đếm hàng + phép đo ô số mới bắt được hàng bị **nhân đôi** |
| TEACHER-REPORT-5d | **Nút "Xuất báo cáo" của giáo viên + tên file PDF** | Bản in của giáo viên là **trang riêng** `/teacher/reports/[sessionId]/ban-in` (in nguyên biểu mẫu 9 mục là in một cái form: 35 ô `<input>` ra khung rỗng, chữ bị cắt ở đáy ô), chỉ nhận báo cáo **đã gửi**. Hai trang dùng **chung** `SessionReportPrintable`. Tên file qua `document.title` — Chrome/Edge/Firefox lấy chuỗi đó làm tên gợi ý **rồi tự thêm `.pdf`**, nên `sessionReportFileBase()` cố tình không có đuôi. Ngày đọc từ **mốc ISO thật** (`startsAtISO`), không bóc ngược chuỗi đã định dạng (`TEACHER-REPORT-4b`) | ☑ **DONE** (phiên 109) — 🔴 kiểm ngược lần đầu **KHÔNG đỏ** vì Storage local trả ảnh quá nhanh; phải `page.route(..., { times: 1 })` làm chậm chính request ảnh 1,5s |
| TEACHER-REPORT-5e | **Lỗ cũ bắt được khi gom hai bản in** | Bản in **thiếu khối XÁC NHẬN** trong khi DOCX có nó — `buildConfirmationLine()` tồn tại mà không bề mặt nào gọi. Bài e2e đếm tiêu đề mục **9 → 10** ghim lại | ☑ **DONE** (phiên 109) |
...
**Cổng đã chạy thật (phiên 106):** lint **0** · typecheck **0** · Vitest **621/621** (608 → +13) · build **exit 0** · pgTAP **748/748** trên DB sạch · Playwright `session-reports` **5/5** trên Chromium. `TEACHER-REPORT-3` **KHÔNG có migration** — ảnh vốn nằm sẵn trong bucket và bảng, lỗi hoàn toàn ở tầng đọc/dựng nội dung. Migration `…092` và `…093` của các phiên trước **đều đã áp cloud**; `db push --dry-run` lần hai = *"Remote database is up to date."*
...
**Cổng đã chạy thật (phiên 111):** lint **0** · typecheck **0** · Vitest **652/652** (647 → +5) · build **exit 0**. ⚠️ **Không** chạy pgTAP/Playwright — Docker Desktop không chạy trên máy phiên này. **Không có migration.**

🔴 **Không có DB local hoá ra không phải rào cản.** Chế độ `-v apply=false` **chạy trọn script trên chính production rồi `ROLLBACK`** — kiểm cú pháp, cổng an toàn và dữ liệu thật cùng một lượt, mạnh hơn hẳn một lượt thử trên bản sao local. 📌 Mọi script sửa dữ liệu về sau nên có sẵn chế độ này.
