...
| Phân cấp            | `deck` (**nhiều bộ / 1 khóa học** từ `MULTIDECK-1`, mỗi bộ có `code` riêng) → `section` (buổi, có `session_number`) → `page` (trang) | `flashcard_decks` / `_sections` / `_pages` trong `src/types/database.ts:1398+`  |
