...
- Sau Phase 14, production dùng 56 bảng public đều RLS, 65 RPC public và 6 bucket nghiệp vụ private; assessment dùng `question-media` + `answer-media`, Flashcard dùng `flashcard-media`.
...
| EX-16 | ~~Giáo viên có thể nhập câu hỏi hàng loạt bằng file Excel mẫu.~~ **ĐÃ ĐẢO (2026-07-17):** gỡ chức năng import Excel khỏi UI; RPC cũ không có lối vào người dùng. |
...
| EX-21 | Màn chấm Bài tập/Thi dành cho giáo viên không hiển thị JSON, enum hoặc thuật ngữ kỹ thuật. Câu thủ công chưa nhập điểm là **Chưa chấm**; một nút lưu toàn bộ phần đã nhập bằng transaction và cảnh báo nếu còn sót. |
| EX-22 | Danh sách bài/kỳ thi đã giao chia theo lớp, trình bày dạng dòng gọn. Kết quả học viên hiển thị câu hỏi, nội dung lựa chọn, điểm, đáp án và nhận xét ở dạng đọc được; không lộ khóa/payload kỹ thuật. |
| EX-23 | Website cho phép micro cho chính origin bằng Permissions Policy `microphone=(self)`, kiểm tra trước câu Nói và phân biệt lỗi cấu hình website với lỗi quyền site/trình duyệt/hệ điều hành. |
...
Ở màn quản lý bộ, Preview mở trong một cửa sổ riêng có vùng cuộn để giáo viên
xem trọn đề; không xả toàn bộ renderer xuống trang danh sách. Bảng chọn câu phải
ghi rõ câu nào đã nằm trong bộ hiện tại, kể cả ngân hàng đã chuyển sang version
mới của chính câu đó. Điểm câu không có giá trị mặc định: giáo viên phải nhập
trước khi thêm. Với bài tập, `max_score` khi giao luôn lấy từ `raw_max_score` của
bộ đã khóa, không cho nhập một tổng điểm độc lập.
