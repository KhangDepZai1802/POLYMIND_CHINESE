...
- `flashcard-media` cũng private. Upload theo `{admin_uid}/{deck_id}/{section_id}/{page_id}/{slot}-{uuid}.{ext}`; server kiểm lại object cho cả `front/back/audio`. Student chỉ được `createSignedUrls` khi page/section/deck đã publish và Course khớp enrollment qua Storage RLS.
