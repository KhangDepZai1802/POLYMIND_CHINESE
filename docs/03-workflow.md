...
Super Admin mở /admin/flashcards
  ...
  → tạo page id + server ký vé upload: trang mở đầu 2 vé (front/back), trang từ vựng 3 vé (front/back/audio)
...
Học viên mở /student/review, tab Flashcard
  ...
  → click/Enter/Space lật front/back dưới↔trên quanh tâm; ảnh hiện trọn khung trên desktop, không có overlay nhãn/từ trên thẻ
  → audio là một nút mang đúng tiêu đề admin đặt cho trang, phát từ signed URL (trang mở đầu không có audio)
