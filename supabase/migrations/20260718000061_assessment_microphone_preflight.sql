-- 61 — Báo trước kỳ thi có câu Nói để phòng chờ kiểm tra quyền micro trước khi timer chạy
  ...
  return jsonb_build_object(
    ...
    'exams', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'requires_microphone', exists(
          select 1
          from public.question_set_items i
          join public.question_versions qv on qv.id = i.question_version_id
          where i.set_version_id = d.set_version_id and qv.question_type = 'speaking'
        ),
        'class', jsonb_build_object('code', c.code, 'name', c.name),
        'attempts', coalesce((select jsonb_agg(jsonb_build_object(
