...
test("scenario 4 — admin tạo invoice, thu tiền, sinh receipt; student chỉ xem", async ({
  ...
  /*
   * Đúng lỗi `UX-UIUX-M18-007` lặp lại ở file này: bản cũ dùng
   * `page.once("dialog", d => d.accept())`, mà cách đó chỉ bắt `window.confirm`
   * GỐC của trình duyệt. `IssueInvoiceButton` dùng `useConfirmSubmit` →
   * `ConfirmationProvider` (AlertDialog của Radix), nên dòng đó không bắt gì
   * cả: hộp xác nhận mở ra, không ai bấm, `issueTuitionInvoiceAction` **không
   * bao giờ chạy**, và mọi assertion phía sau (thu tiền, sinh receipt, học viên
   * chỉ xem) chưa từng được thực thi.
   *
   * Bấm đúng như người dùng thật: mở hộp xác nhận rồi xác nhận (`P17-T5`).
   */
