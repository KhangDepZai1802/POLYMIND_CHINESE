...
describe("ExamIntegrityBoundary", () => {
  ...
  it.each(["copy", "cut", "paste", "drop"])(
    "chặn %s và chỉ log loại sự kiện",
    async (type) => {
      render(
        <ExamIntegrityBoundary attemptId="attempt-1">
      ...
      await waitFor(() =>
        expect(logExamEvent).toHaveBeenCalledWith(
          "attempt-1",
          type === "drop" ? "paste_blocked" : `${type}_blocked`,
        ),
      );
  ...
  it("không chặn sự kiện trong lúc IME đang composition", () => {
    render(
      <ExamIntegrityBoundary attemptId="attempt-2">
        <input aria-label="中文答案" />
      </ExamIntegrityBoundary>,
    );
  ...
  it("bật chế độ tập trung và cảnh báo khi tải lại trong lúc thi", () => {
    const { unmount } = render(
      <ExamIntegrityBoundary attemptId="attempt-3">
        <p>Đề thi</p>
      </ExamIntegrityBoundary>,
    );
    ...
    unmount();
    expect(document.documentElement).not.toHaveAttribute("data-exam-active");
  });
});
