...
> Cập nhật: **2026-07-18** — Claude — Làm lại luồng 3 tab Bài tập/Thi cho dễ dùng: đánh số tab theo luồng + StepHint, card câu hỏi gọn (bỏ Lưu trữ, gom vào kebab), bảng chọn câu hỏi có tìm kiếm + chọn nhiều. Lint/typecheck sạch · Vitest 100/100 · build xanh; chưa smoke trình duyệt (Phiên 34)
...
**Smoke P-C (migration 54–55 đã áp cloud).** Bước tiếp: (1) commit + push fix `sidebar-nav.tsx` để Vercel redeploy hết 500 (nếu chưa). (2) Smoke Nói đủ luồng trên prod: soạn câu Nói (wizard), HV thu âm/nghe lại/xóa & thu lại/nộp, GV nghe + chấm điểm tự do, công bố; đồng thời xác minh HV **nghe được audio Nghe/Chép** (signed URL). Song song: `P7-T8` (admin cấp tài khoản) → `P7-T9` → smoke `P7-T7`.
